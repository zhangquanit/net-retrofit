...
final class ServiceMethod<T> {
  ...
  final okhttp3.Call.Factory callFactory; //其实就是OkHttpClient ，OkHttpClient 实现了Call.Factory接口
  final CallAdapter<?> callAdapter; // ExecutorCallAdapterFactory

  private final HttpUrl baseUrl;
  private final Converter<ResponseBody, T> responseConverter; //http响应转换器
  private final String httpMethod; // http请求方式，比如GET，POST
  private final String relativeUrl; //相对url ，比如 article/details/11
  private final Headers headers; // 设置的 请求消息头
  private final MediaType contentType; //
  private final boolean hasBody;
  private final boolean isFormEncoded;
  private final boolean isMultipart;
  private final ParameterHandler<?>[] parameterHandlers; //方法参数处理器(根据方法参数上的注解信息，用于构造最终请求)
  ...
  static final class Builder<T> {
    ...
    public ServiceMethod build() {
      //创建CallAdapter，默认是ExecutorCallAdapterFactory创建的
      callAdapter = createCallAdapter();
      responseType = callAdapter.responseType(); // Call<?>中的反省参数?
    ...
    //p 参数index，parameterType：参数值类型，annotations：参数上使用的注解
